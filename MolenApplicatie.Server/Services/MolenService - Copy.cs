    ...
    public class MolenService2_0
    {
        ...
        public static MolenData GetMolenData(MolenData molen)
        {
            molen.ModelTypes = molen.MolenTypeAssociations
                .Where(mta => mta.MolenType != null)
                .Select(mta =>
                {
                    mta.MolenType.MolenTypeAssociations = null;
                    return mta.MolenType;
                }).ToList();
        ...
        public async Task<MolenData?> GetMolenByTBN(string tbn)
        {
            var molen = await _dbContext.MolenData
                ...
                .Include(m => m.MolenMakers)
                .Include(m => m.DisappearedYearInfos)
                .FirstOrDefaultAsync(m => m.Ten_Brugge_Nr.ToLower() == tbn.ToLower());
