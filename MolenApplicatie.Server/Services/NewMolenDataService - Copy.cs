...
{
    public class NewMolenDataService2_0
    {
        ...
        public NewMolenDataService2_0(MolenService2_0 molenService, PlacesService2_0 placesService, MolenDbContext dbContext)
        {
        ...
        public async Task<List<MolenData>?> AddMolensToMariaDb(List<MolenData> molens)
        {
            ...
            foreach (var molen in molens)
            {
                ...
                await AddDisappearedYearInfosAsync(molen, molenData);
        ...
        private async Task AddMakersAsync(MolenData molen, MolenData molenData)
        {
        ...
        private async Task AddDisappearedYearInfosAsync(MolenData molen, MolenData molenData)
        {
            if (molen.DisappearedYearInfos == null) return;
            foreach (var year in molen.DisappearedYearInfos)
            {
        ...
        private async Task AddAddedImagesAsync(MolenData molen, MolenData molenData)
        {
        ...
        private async Task AddModelTypesAsync(MolenData molen, MolenData molenData)
        {
            if (molen.ModelTypes == null) return;
            foreach (var type in molen.ModelTypes)
            {
        ...
        private async Task AddMolenImagesAsync(MolenData molen, MolenData molenData)
        {
        ...
        public async Task<MolenData?> AddMolenToMariaDb(MolenData molen)
        {
            ...
            {
                foreach (var yearInfo in molen.DisappearedYearInfos)
                {
            ...
            if (molen.ModelTypes != null)
            {
                foreach (var MolenType in molen.ModelTypes)
                {
                    MolenType? existingType = _dbContext.ChangeTracker.Entries<MolenType>()
                        .Select(e => e.Entity)
                        .FirstOrDefault(t => t.Name == MolenType.Name);

                    if (existingType == null)
                    {
                        existingType = await _dbContext.MolenTypes.FirstOrDefaultAsync(t => t.Name == MolenType.Name);
                    ...
                    {
                        existingType.Name = MolenType.Name;
        ...
        {
            List<MolenType> MolenTypes = await _dbContext.MolenTypes.ToListAsync();
            ...
            MolenRemovedYears = oldMolenData?.DisappearedYearInfos ?? new List<DisappearedYearInfo>();
            try
            {
                ...
                if (divs != null)
                {
                    ...
                    MolenData newMolenData = new MolenData() { Id = -1, Name = "", Ten_Brugge_Nr = Ten_Brugge_Nr, DisappearedYearInfos = MolenRemovedYears, Images = new List<MolenImage>() };
                    foreach (var div in divs)
                    {
                        ...
                        if (!string.IsNullOrEmpty(dt) && !string.IsNullOrEmpty(dd))
                        {
                            ...
                            switch (dt.ToLower())
                            {
                                ...
                                case "geschiedenis":
                                    ...
                                    if (imageInGeschiedenis.isAlreadyInDB && imageInGeschiedenis.Image != null)
                                    {
                                        _dbContext.MolenImages.Update(imageInGeschiedenis.Image);
                                    }
                                    break;
                        ...
                        else if (isNogWaarneembaarPrevious)
                        {
                            ...
                            if (nogWaarneembareImage.isAlreadyInDB && nogWaarneembareImage.Image != null)
                            {
                                _dbContext.MolenImages.Update(nogWaarneembareImage.Image);
                            }
                        ...
                        foreach (var modelDiv in ModelTypeDiv)
                        {
                            ...
                            if (!string.IsNullOrEmpty(dt) && !string.IsNullOrEmpty(dd))
                            {
                                foreach (string type in dd.Split(','))
                                {
                                    ...
                                    {
                                        await _dbContext.MolenTypes.AddAsync(molenType);
                                        NewAddedTypes.Add(molenType);
                                        newMolenData.ModelTypes.Add(molenType);
                                    }
                                    else
                                    {
                                        var existingType = newMolenData.ModelTypes.Find(x => x.Name == molenType.Name);
                                        if (existingType == null)
                                        {
                                            newMolenData.ModelTypes.Add(new MolenType { Name = molenType.Name });
                                        }
                                    }
                                }
                                NewAddedTypes.AddRange(newMolenData.ModelTypes);
                            }
                        }
                    ...
                    if (newMolenData.Images.Count < Globals.MaxNormalImagesCount)
                    {
                        ...
                        if (ArticleAbout != null)
                        {
                            ...
                            if (images != null)
                            {
                                ...
                                if (molenImage.isAlreadyInDB && molenImage.Image != null)
                                {
                                    _dbContext.MolenImages.Update(molenImage.Image);
                                }
                            }
                        }
                    ...
                    {
                        await _dbContext.MolenData.AddAsync(newMolenData);

                        foreach (var year in newMolenData.DisappearedYearInfos)
                        {
                            var molenYearInfo = new DisappearedYearInfo
                            {
                                Status_before = year.Status_before,
                                Year = year.Year,
                                Status_after = year.Status_after,
                            ...
                            await _dbContext.DisappearedYearInfos.AddAsync(molenYearInfo);
                        }
                    ...
                    {
                        _dbContext.MolenData.Update(newMolenData);

                        var molenYearInfoList = MolenRemovedYears
                            .Select(year => new DisappearedYearInfo
                            {
                                Status_before = year.Status_before,
                                Year = year.Year,
                                Status_after = year.Status_after,
                        ...
                        List<(int, DisappearedYearInfo)> updatedYears = new List<(int, DisappearedYearInfo)>();
                        foreach (var molenYearInfo in oldMolenData.DisappearedYearInfos)
                        {
                            int index = newMolenData.DisappearedYearInfos.FindIndex(year => year.Status_before == molenYearInfo.Status_before && year.Year == molenYearInfo.Year && year.Status_after == molenYearInfo.Status_after);
                            if (index == -1)
                            {
                                _dbContext.DisappearedYearInfos.Remove(molenYearInfo);
                            }
                            else
                            {
                        ...
                        {
                            oldMolenData.DisappearedYearInfos[index] = molenYearInfo;
                        }
                        ...
                        foreach (var molenmaker in newMolenData.MolenMakers)
                        {
                            ...
                            else
                            {
                                ...
                                molenmaker.MolenDataId = newMolenData.Id;
                                _dbContext.MolenMakers.Update(molenmaker);
                            }
                        }
                    }

                    var allExistingTypes = await _dbContext.MolenTypeAssociations
                        .Where(x => x.MolenDataId == newMolenData.Id)
                        .ToListAsync();

                    foreach (var type in newMolenData.ModelTypes)
                    {
                        if ((allExistingTypes != null && allExistingTypes.Find(x => x.MolenTypeId == type.Id) == null) || allExistingTypes == null)
                        {
                            ...
                            if (foundType == null)
                            {
                                await _dbContext.MolenTypes.AddAsync(type);
                            }
                            ...
                            {
                                await _dbContext.MolenTypeAssociations.AddAsync(new MolenTypeAssociation() { MolenDataId = newMolenData.Id, MolenTypeId = type.Id });
                            }
                        }
                    }
                    for (int i = 0; i < newMolenData.Images.Count; i++)
                    {
                        ...
                        if (!isImageInOldMolen)
                        {
                            ...
                            if (!File.Exists(Globals.WWWROOTPath + newMolenData.Images[i].FilePath) && Uri.IsWellFormedUriString(newMolenData.Images[i].ExternalUrl, UriKind.Absolute))
                            {
                                ...
                                await _dbContext.MolenImages.AddAsync(newMolenData.Images[i]);
                            }
                        ...
                        else if (imageInOldMolen != null)
                        {
                            ...
                            {
                                _dbContext.MolenImages.Update(new MolenImage()
                                {
                    ...
                    if (Directory.Exists(Globals.MolenAddedImagesFolder) && Directory.Exists(Globals.MolenAddedImagesFolder + "/" + Ten_Brugge_Nr))
                    {
                        ...
                        foreach (var image in allImages)
                        {
                            ...
                            if (newMolenData != null && newMolenData.AddedImages.Find(x => x.Name == imageName && x.FilePath == imagePath) == null)
                            {
                                ...
                                await _dbContext.AddedImages.AddAsync(newAddedMolenImage);
                    ...
                    if (oldMolenData != null)
                    {
                        ...
                        if (somethingChanged && newMolenData != null)
                        {
                            _dbContext.MolenData.Update(newMolenData);
                        }
                    }
        ...
        public async Task<(bool isAlreadyInDB, MolenImage? Image)> GetImageFromHtmlNode(HtmlNode dd2, string Ten_Brugge_Nr, string filePath, string? description = null, bool canBeDeleted = false)
        {
            ...
            if (nogWaarneembareImages != null && nogWaarneembareImages.Count > 0)
            {
                ...
                if (!string.IsNullOrEmpty(nogWaarneembareImageSrc) && Uri.IsWellFormedUriString(nogWaarneembareImageSrc, UriKind.Absolute))
                {
                    ...
                    var foundImagesInDB = await _dbContext.MolenImages
                        .Where(x => x.ExternalUrl == nogWaarneembareImageSrc)
                        .ToListAsync();
                    ...
                    using (var md5 = MD5.Create())
                    {
                        ...
                        else if (existingPath != null)
                        {
                            ...
                            foundImagesInDB = await _dbContext.MolenImages
                                .Where(x => x.FilePath == pathWithoutRoot)
                                .ToListAsync();
